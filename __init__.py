...
def add_bill():
    if authorize.check_login(session):
        if request.method == 'POST':
            ...
            post['billing_date'] = datetime.datetime.strptime(request.form['billing_date'], '%Y-%m-%d')
            ...
            if post['billing_date'].month == post['due_date'].month:
                for i in range(1, 13):
                    post['billing_date'] = datetime.date(post['billing_date'].year, i, post['billing_date'].day)
                    ...
                    c.execute(
                        ...
                        [last_bill_entered['bill_id'], post['due_date'], post['billing_date'], post['due_date']])
            else:
                for i in range(1, 13):
                    post['billing_date'] = datetime.date(post['billing_date'].year, i, post['billing_date'].day)
                    if i == 12:
